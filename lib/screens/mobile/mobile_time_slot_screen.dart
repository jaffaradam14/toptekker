...
class TimeSlotScreenPageState extends State {
  ...
  final List<SlotModel> timeslotmorningdata = [];
  ...
  Widget _listViewTimeSlotItemBuilder(BuildContext context, int index) {
    SlotModel newsDetail = this.timeslotmorningdata[index];
                    ...
                    setState(() {
                      ...
                      if (index > 0) {
                        if(!timeslotmorningdata[index].isSelected){
                          if(timeslotmorningdata[index - 1].isSelected){
                            timeslotmorningdata[index].isSelected = true;
                          }else{
                            for (int i = 0; i <= index; i++) {
                              if (timeslotmorningdata[i].isSelected) {
                                timeslotmorningdata[i].isSelected = false;
                              }
                            }
                            timeslotmorningdata[index].isSelected = true;
                          }
                      ...
                      selectedIndex = selectedTimeSlots.indexOf(newsDetail);

                      if (selectedIndex == 0 ||
                          selectedIndex == selectedTimeSlots.length - 1) {
                        timeslotmorningdata[index].isSelected = false;
                      } else if (selectedIndex >
                          (selectedIndex - selectedTimeSlots.length).abs()) {
                        selectedTimeSlots
                            .sublist(selectedIndex, selectedTimeSlots.length)
                            .clear();
                      } else if (selectedIndex >
                              (selectedIndex - selectedTimeSlots.length)
                                  .abs() ||
                          selectedIndex >
                              selectedIndex -
                                  (selectedTimeSlots.length).abs()) {
                        selectedTimeSlots.sublist(0, selectedIndex + 1).clear();
                      }*/
                    });
                  },
                ))
  ...
  Widget _itemTimeSlotThumbnail(
      SlotModel newsDetail, String booking_id, int index) {
  ...
  Widget _itemTimeSlotTitle(SlotModel newsDetail) {
    return Container(
        padding: EdgeInsets.fromLTRB(0, 0, 0, 10),
        child: Text(
          newsDetail.price,
    ...
    if (response.statusCode == 200) {
      ...
      if (!responseData['data'].isEmpty) {
        responseData['data']['morning'].forEach((morningData) {
          final SlotModel timeSlotMorningData = new SlotModel(
            morningData['slot'],
